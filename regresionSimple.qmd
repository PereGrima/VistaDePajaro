...
Por tanto, la suma de los cuadrados de los residuos, $S$, será:

$$ S = \sum_{í=1}^n \left(y_i - b_0 - b_1 x_i \right )^2 $$
Tanto los valores de $y_i$ como los de $x_i$ vienen dados. La suma de cuadrados $S$ es función de los valores de $b_0$ y de $b_1$ y podemos escribir $S \left(b_0, b_1 \right )$ y se trata de hallar los valores de $b_0$ y de $b_1$ que minimizan la suma de cuadrados. El mínimo lo tendremos en el punto en que la derivada de $S \left(b_0, b_1 \right )$ respecto a $b_0$ y respecto a $b_1$ es igual a cero. También habrá que verificar que es un mínimo comprobando que la segunda derivada es un valor positivo.
...
También la expresión de $b_1$ se suele dar de la forma:

$$ b_1 = \frac{\sum (x_i - \bar{x})(y_i - \bar{y})}{\sum (x_i - \bar{x})^2} $$ {#eq-expresion_b1}
Las dos expresiones son equivalentes ya que en el numerador:
...
$$ b_1 = \frac{Cov(XY)}{s_X^2} = \frac{r_{XY} s_X s_Y}{s_X^2} = r_{XY} \frac{s_Y}{s_X} $$
Calculando los coeficientes que corresponden a los datos de la figura [-@fig-fuerzaBruta] se obtiene: 


$$ b_0 = 26,9615 \qquad  \qquad b_1 = 4,8616 $$
redondeando estos resultados con un solo decimal, el valor de $b_0$ queda igual a 27,0 que es el valor que habíamos obtenido por el método que hemos denominado "fuerza bruta", pero $b_1$ queda igual a 4,9 y antes nos había salido 4,8. ¿Por qué?


::: callout-note
## Curiosidad sobre el redondeo de los coeficientes del modelo
