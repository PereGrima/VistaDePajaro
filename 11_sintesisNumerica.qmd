...
Cuando tenemos un conjunto de datos y nos interesa conocer la varianza de **esos datos** dividimos por el número de los que tenemos, pero aunque esta parede la situación más habitual, no es así. Lo habitual es que nuestro interés no sea tanto conocer la varianza de los datos dispobles como estimar la varianza de la población de la cual provienen y de la que solo tenemos unos representantes.

En este caso tampoco se conoce la media de la población de manera que debemos sustituir su valor por la media de la muestra ($\bar{x}$). Esta sustitución tiene más influencia de la que parece ya que la media de los datos disponibles se adapta a esos datos y la variabilidad obtenida tiende a subestimar la varianza de la población. Puede demostrarse que se si en vez de dividir por el número de datos $n$ (al número de elementos de la muestra le llamamos $n$ minúscula) subestima la variabilidad de la población. Puede demostrarse que si en vez de dividir por $n$ se divide por $n-1$ esa subestimación --se llama *sesgo*-- ya no se da. Esto es especialmente importante cuando se tienen muestras pequeñas ya que puede haber una diferencia importante entre dividir por $n$ o hacerlo por $n-1$.
...
+--------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------+
| **PROS** <i class="fa-solid fa-thumbs-up fa-xl" style="color: #0ca701;"></i>   | -   Excelentes propiedades que no tieneninguna otra medida.                                               |
...
Tiene la ventaja de que sus unidades son las mismas que las de los datos. El precio a pagar es que la raiz cuadrada estropea las propiedades algebraicas que tiene la varianza, por lo que en los desarrollos teóricos en muy habitual que el protagonismo lo tenga la varianza.

La desviación típica, junto con la media, forman una pareja perfecta para describir el comportamiento de muchas variables aleatorias. En particular, forman parte del *ADN* de la distribución Normal. 
...
Si la variable considerada no sigue una distribución Normal ¿sigue siendo útil la desviación típica? La respuesta es sorprendente. Para una amplia gama de distribuciones de probabilidad se pueden responder preguntas de naturaleza similar usando la llamada "desigualdad de Chebyshev" [^2] 

[^2]: La idea es que para una amplia variedad de distribuciones, en el intervalo $\mu \pm k\sigma$  se tiene como mínimo una proporción de observaciones igual a $1-1/k^2$. Es decir, si $k=\sqrt{2}$, el intervalo es $\mu \pm \sqrt{2}\sigma$ y podemos afirmar que, con independencia del tipo de distribución, tendremos como mínimo el 50% de las observaciones dentro de ese intervalo, y si $k=2$ tendremos como mínimo el 75%. Una curiosa propiedad en la que nuestra pareja media--desviación típica también es protagonista.

En muchas situaciones tenemos interés en estimar el valor de la media poblacional $\mu$. Para ello tomamos una muestra y calculamos su media  $\bar{x}$ para hacernos una idea del valor desconocido del parámetro $\mu$. El problema es que cada vez que se tome una muestra, el valor de la media muestral $\bar{x}$ cambiará, pero sabemos que si tomamos muchas muestras de tamaño $n$, sus medias estarán alrededor del verdadero valor de la media poblacional $\mu$. Pero decir que están alrededor de $\mu$ no significa que estén cerca de $\mu$. ¿Cómo podemos saber que tan cerca están? En la respuesta a esta pregunta la desviación típica también tiene un papel protagonista. La desviación típica de esas medias muestrales \textbf{siempre} es igual a la desviación típica de la población dividida por $\sqrt{n}$. Esto es verdad para \textbf{todas} las distribuciones, no solo para la Normal.
