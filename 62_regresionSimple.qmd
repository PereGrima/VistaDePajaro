...
Conocer la distribución de los coeficientes permite realizar las pruebas de significación  de manerea más directa, sin necesidad de simular. Pueden no ser necesarias en casos tan claros como los que hemos visto pero, en general, aportan informaciuón muy útil para interpretar el resultado obtenido.
...
##### **Contraste sobre el valor de la ordenada en el origen, $\beta_0$**  {.unnumbered}

En algunos casos, por el conocimiento que se tiene del fenómeno en estudio, es razonable suponer que cuando $X=0$ tendremos $Y=0$, es decir, que la recta pasará por el origen. En este caso, una prueba de significación del tipo: $H_0: \beta_0=0$ frente a $H_1: \beta_0 \neq 0$ permite verificar que los resultados obtenidos no están en contradicción con el supuesto realizado.

Para realizar este contraste, como $b_0 \sim N \left(\beta_0; \; \sigma_{b_0} \right)$, si $\beta_0 = 0$ tenemos:

$$ \frac{b_0-0}{\sigma_{b_0}} \sim N (0; 1) $$

Pero no conocemos $\sigma_{b_0}$. En su lugar usamos su valor estimado a través de la varianza de los residuos. Por tanto, respecto al valor y a la distribución del estadístico de prueba, $T$, tenemos::

$$ T = \frac{b_0}{s_{b_0}} \sim t-\text{Student} \;\text{con} \; n-2 \; \text{grados de libertad} $$ 

Los grados de libertad de la distribución $t-$Student tienen que ver con las restricciones que presentan los residuos cuando el modelo se ha ajustado por el método de los mínimos cuadrados $\left ( \sum e_i = 0 \; \text{y} \; \sum e_ix_i = 0 \right )$.
...
##### **Contraste sobre el valor de la pendiente de la recta, $\beta_1$** {.unnumbered}
...
También puede interesar contrastar otros valores para la pendiente de la recta. Por ejemplo:

![Distribución de los coeficientes.](regresionSimple/220_pendiente1.png){#fig-pendiente1 fig-align="center" width="100%"}


##### **Interpretación de los resultados presentados por los paquetes de software estadístico** {.unnumbered}
