...
Volvamos a los datos del volumen de madera que se puede obtener de un árbol en función del diámetro del tronco y los datos de la edad al morir en función de la longitud de una línea de la mano. La figura [-@fig-modelosAjustados] contiene las rectas ajustadas, el número de puntos $(n)$ y el valor de la desviación típica de los residuos $(s_R)$ en cada caso.

![Modelos ajustados. Los diagramas incluyen el valor de la desviación típica de los residuos.](regresionSimple/200_ArbolManoModelos.png){#fig-modelosAjustados fig-align="center" width="100%"}

Si no hay ninguna relación entre la variable regresora ($X$) y la respuesta ($Y$), la pendiente de la recta estará en torno a cero (sería igual a cero en la población, pero no disponemos de esos datos que --además-- son solo un modelo teórico). Para ver lo que significa ese "en torno a cero" en los casos que estamos tratando, podemos simular nubes de puntos suponiendo que no existe ninguna relación entre $X$ e $Y$ manteniendo las características de estas variable en los ejemplos que estamos tratando.

Para realizar la simulación, en el caso del volumen de madera, para el diámetro de los troncos ($X$) generamos 31 números aleatorios (una cantidad igual a nuestro tamaño de muestra) de una distribución Normal con la media y la desviación típica de los valores que aparecen en la muestra, es decir: $X \sim N(13.25; 3.2)$. A cada valor de $X$ de corresponderá un valor de la respuesta igual a la media de los valores e $Y$ --tomaremos de de los datos disponibles-- añadiendo una variabilidad aleatoria que introducimos sumando al valor de Ybarra un número aleatorio de una distribución Normal con media cero (ni sube ni baja de forma sistemática el valor de la respuesta) y una desviación típica igual a la que presentan los residuos del modelo ajustado. Por tanto, tendremos: $Y = 30.2 + N(0; 4.25)$.

Para el modelo de la edad al morir hacemos exactamente lo mismo con los datos que corresponden. La tabla [-@tbl-parametrosSimulacion] contine los valores usados para la simulación en cada caso.
...
Para conjunto de puntos simulados se ha calculado la recta ajustada. En los dos casos la simulación se ha repetido 50 veces y se han ido acululando, tanto los puntos como las rectas ajustadas, en los diagramas de la figura [-@fig-pruebasSignificacion]. Está muy claro que en el caso del volumen de madera, la recta ajustada con nuestros datos no se puede confundir con los generados aleatoriamente suponiendo que no hay relación entre ambas variables, sin embargo....
