...
png("boxplotEstratificado.png", width=1250, height=1250, res = 300)

par(mar = c(1, 2, 0.2, 0), fig=c(0, 0.55, 0.5, 1))
boxplot(expVida ~ continente, x,
        col = "white", medcol="white", staplecol="white",
        boxcol = "white", whiskcol = "white",
        outline = F, cex=t, ylab = "", xlab = "", ylim = c(30, 90), 
...
stripchart(expVida ~ continente,
           ...
           col = 2:4,
           vertical = TRUE,
           add = TRUE, cex=t,  xaxt='n', yaxt='n',  xaxs="i", yaxs="i")
axis(1, at=1:5, labels = c("Africa", "Améri.", "Asia", "Euro", "Ocean."), tck=-0.02, las=1, cex.axis=t-0.05, padj = -2.8)
axis(2, seq(30, 90, by=10), tck=-0.02, las=1, cex.axis=t-0.07, hadj = -0.4, xpd = NA)
mtext(side = 2, text = "Esperanza de vida (años)", line = 1, cex=t-0.07)

par(mar = c(1, 0.2, 0.2, 0), fig=c(0.55, 1, 0.5, 1), new = TRUE)
boxplot(expVida ~ continente, x,
        medlwd=1, whisklty = 1, cex=t, ylab = "", xlab = "", ylim = c(30, 90),
        xaxt='n', yaxt='n',  xaxs="i", yaxs="i", main = "")
axis(1, at=1:5, labels = c("Africa", "Améri.", "Asia", "Euro", "Ocean."), tck=-0.02, las=1, cex.axis=t-0.05, padj = -2.8)

##########################

par(mar = c(1, 2, 0.2, 0), fig=c(0, 0.55, 0, 0.5), new = TRUE)
...
par(mar = c(1, 0.2, 0.2, 0), fig=c(0.55, 1, 0, 0.5), new = TRUE)
boxplot(PBI_PC ~ continente, x,
